...
Every page in `docs/` has a Russian counterpart alongside it (`*.ru.md`). **The English is
the source of truth**: when the two disagree, the English is right and the Russian is a bug.
That is not a preference, it is the only arrangement under which a translation cannot quietly
become a second, wrong set of measurements — and `tools/check_docs_parity.py` enforces the
part a machine can check.
...
succeeded. It used to be zero for every outcome, so
`openm74 --flash img.bin --yes && echo ok` printed `ok` over an ECU that had just been left
unwritten — worth knowing if you have a script written against an older build.
...
The documentation check compares what a translation has no business changing: numbers
written as digits, hex addresses, anything in backticks, and link targets. It cannot read
prose and does not try. Every documentation drift this project actually suffered was one of
those four — a per-sector cost that said 2.05 s in one place and 2.64 in another, a probe
count of 48 against 16, an English help text still promising "about an hour" where the
Russian said thirteen minutes. A human reading either half alone caught none of them.
