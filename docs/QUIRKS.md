...
**Unplugging the USB end alone does not clear it**, and this file used to say it would —
a sentence carried over from the hang above without being tested against this symptom.
Measured: the adapter was unplugged and reconnected, the device node was recreated with a
fresh timestamp, no process held it, no third-party driver was installed, and `EINVAL` came
straight back. That is the observation the paragraph above explains.

If a full power-down does not clear it, escalate in this order:
...
The ECU is not at fault in any of this — but note that its supply is part of the cure rather
than part of the diagnosis: cutting it is how the adapter loses power, not something the ECU
needs. Re-seating the OBD end changes nothing.
